...
# Format bar labels: show all values
bar_label <- function(x) sprintf("%.2f", x)
...
make_tpr_plot <- function(data, title_text) {
  ggplot(data, aes(x = model, y = score, fill = path)) +
    ...
    geom_text(aes(label = bar_label(score), y = max + 0.04),
              position = position_dodge(0.6),
              vjust = 0, size = 2.1, color = "#888888") +
...
ggsave(filename = pdf_out, plot = p_final, width = 10, height = 10,
       device = "pdf", bg = "white")
ggsave(filename = png_out, plot = p_final, width = 10, height = 10,
