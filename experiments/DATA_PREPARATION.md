...
Then the universe is built once using `universe_generator.py` (see `00_create_universe.sh`).
...
Script: `preproc_tsv.py` (also available in `experiments/preproc_tsv.py`)
...
Six universes are built (one per gene set category) using `04_panukb_universes.sh`.
...
4. Run `00_create_universe.sh` to build the universe
5. Run `01_lsea_continuous.sh`, `02_lsea_binary.sh`, `03_lsea_sensitivity.sh`
...
5. Run `04_panukb_universes.sh` to build 6 universes
6. Run `05_panukb_enrichment.sh` (Experiment 1)
7. Run `06_panukb_gwas_on_gwas.sh` (Experiment 2)
