#!/bin/bash
# ============================================================
# validate_single_runs.sh — Run ONE test per experiment type
# ============================================================
# Run this on the server AFTER pulling the updated code.
# It runs a single LSEA analysis for each experiment type and
# compares annotation_stats with existing results.
#
# Usage:
#   cd /path/to/LSEA
#   bash experiments/validate_single_runs.sh
# ============================================================

set -euo pipefail

LSEA_DIR="$(cd "$(dirname "$0")/.." && pwd)"
echo "LSEA_DIR=${LSEA_DIR}"
echo "Date: $(date)"
echo "Git commit: $(git -C "${LSEA_DIR}" rev-parse HEAD)"
echo ""

# --- Common paths ---
PLINK_DIR=/home/achangalidi/tools/plink
OLD_DATA_DIR=/media/DATA/gwasim/round2/bioGWAS/tests
DATA_DIR=/media/DATA/gwasim/round2/lsea_test
BFILE=${OLD_DATA_DIR}/3_pathways/in_data/test10000_filt_sim
UNIVERSE=${DATA_DIR}/in_data/uni.json
PVAL=0.00000000729730

VALIDATION_DIR=${DATA_DIR}/validation_NEW_single
mkdir -p "${VALIDATION_DIR}"

PASS=0
FAIL=0

compare_stats() {
    local label="$1"
    local old_dir="$2"
    local new_dir="$3"

    echo "--- Comparing: ${label} ---"
    for old_stats in "${old_dir}"/annotation_stats_*.tsv; do
        [ -f "${old_stats}" ] || continue
        fname=$(basename "${old_stats}")
        new_stats="${new_dir}/${fname}"
        if [ ! -f "${new_stats}" ]; then
            echo "  MISSING: ${new_stats}"
            FAIL=$((FAIL + 1))
            continue
        fi
        if diff -q "${old_stats}" "${new_stats}" > /dev/null 2>&1; then
            echo "  OK: ${fname}"
            PASS=$((PASS + 1))
        else
            echo "  DIFF: ${fname}"
            diff "${old_stats}" "${new_stats}" || true
            FAIL=$((FAIL + 1))
        fi
    done
    echo ""
}

# ============================================================
# Test 1: Continuous trait (test10000_path_small_0)
# ============================================================
echo "========== Test 1: Continuous trait =========="
TEMPLATE="test10000_path_small_0_path_small_0"
GWAS=${OLD_DATA_DIR}/3_pathways/extra_in_data/${TEMPLATE}_gwas.tsv
OLD_OUT=${DATA_DIR}/lsea_results/${TEMPLATE}
NEW_OUT=${VALIDATION_DIR}/${TEMPLATE}

python3 "${LSEA_DIR}/LSEA_2.4.py" \
    --input "${GWAS}" \
    --universe "${UNIVERSE}" \
    --out "${NEW_OUT}" \
    --plink_dir "${PLINK_DIR}" \
    --bfile "${BFILE}" \
    --column_names chr pos rsid pval \
    --clump_p1 "${PVAL}"

compare_stats "Continuous (path_small_0)" "${OLD_OUT}" "${NEW_OUT}"

# ============================================================
# Test 2: Binary trait (bin10000_path_small_0)
# ============================================================
echo "========== Test 2: Binary trait =========="
TEMPLATE="bin10000_path_small_0_path_small_0"
GWAS=${OLD_DATA_DIR}/3_pathways/binary_in_data/${TEMPLATE}_gwas.tsv
OLD_OUT=${DATA_DIR}/lsea_results/${TEMPLATE}
NEW_OUT=${VALIDATION_DIR}/${TEMPLATE}

python3 "${LSEA_DIR}/LSEA_2.4.py" \
    --input "${GWAS}" \
    --universe "${UNIVERSE}" \
    --out "${NEW_OUT}" \
    --plink_dir "${PLINK_DIR}" \
    --bfile "${BFILE}" \
    --column_names chr pos rsid pval \
    --clump_p1 "${PVAL}"

compare_stats "Binary (path_small_0)" "${OLD_OUT}" "${NEW_OUT}"

# ============================================================
# Test 3: Sensitivity k=15 (k10000_path_small_k15_0)
# ============================================================
echo "========== Test 3: Sensitivity k=15 =========="
TEMPLATE="k10000_path_small_k15_0_path_small_k15_0"
GWAS=${OLD_DATA_DIR}/3_pathways/extra_in_data/${TEMPLATE}_gwas.tsv
OLD_OUT=${DATA_DIR}/lsea_results/${TEMPLATE}
NEW_OUT=${VALIDATION_DIR}/${TEMPLATE}

python3 "${LSEA_DIR}/LSEA_2.4.py" \
    --input "${GWAS}" \
    --universe "${UNIVERSE}" \
    --out "${NEW_OUT}" \
    --plink_dir "${PLINK_DIR}" \
    --bfile "${BFILE}" \
    --column_names chr pos rsid pval \
    --clump_p1 "${PVAL}" \
    --print_all

compare_stats "Sensitivity k=15 (path_small_0)" "${OLD_OUT}" "${NEW_OUT}"

# ============================================================
# Test 4: Universe re-generation
# ============================================================
echo "========== Test 4: Universe re-generation =========="
OLD_UNIVERSE=${DATA_DIR}/in_data/uni.json
NEW_UNIVERSE=${VALIDATION_DIR}/uni_test.json

SOME_GWAS=${OLD_DATA_DIR}/3_pathways/in_data/test10000_path_small_4_path_small_4_gwas.tsv
VARIANTS=${DATA_DIR}/in_data/variants.tsv
BED=${DATA_DIR}/in_data/anno.bed
GMT=${DATA_DIR}/in_data/c2.cp.kegg.v2023.1.Hs.symbols.gmt

python3 "${LSEA_DIR}/universe_generator.py" \
    --variants "${VARIANTS}" \
    --features "${BED}" "${GMT}" \
    --interval 500000 \
    --out_json "${NEW_UNIVERSE}"

# Compare universe JSON summaries
python3 -c "
import json, sys
old = json.load(open('${OLD_UNIVERSE}'))
new = json.load(open('${NEW_UNIVERSE}'))
checks = [
    ('interval', old['interval'] == new['interval']),
    ('universe_intervals_number', old['universe_intervals_number'] == new['universe_intervals_number']),
    ('gene_set_dict count', len(old['gene_set_dict']) == len(new['gene_set_dict'])),
    ('features count', len(old['features']) == len(new['features'])),
    ('interval_counts count', len(old['interval_counts']) == len(new['interval_counts'])),
]
all_ok = True
for name, ok in checks:
    status = 'OK' if ok else 'DIFF'
    print(f'  {status}: {name} (old={old.get(name, \"?\")} new={new.get(name, \"?\")})')
    if not ok:
        all_ok = False
if all_ok:
    print('  Universe: IDENTICAL')
else:
    print('  Universe: DIFFERENT — check manually')
    sys.exit(1)
"
echo ""

# ============================================================
# Summary
# ============================================================
echo "=========================================="
echo "Validation summary: PASS=${PASS} FAIL=${FAIL}"
if [ "${FAIL}" -eq 0 ]; then
    echo "ALL TESTS PASSED — safe to run full experiments"
else
    echo "SOME TESTS FAILED — investigate before full run"
fi
echo "Results in: ${VALIDATION_DIR}"
