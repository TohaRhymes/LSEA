#!/bin/bash
# ============================================================
# 07_case_study_o15.sh — FinnGen R9 Case Study (Experiment 4)
# ============================================================
...
if [ ! -f "${OUT_PREFIX}.norm.tsv" ]; then
    python3 "${LSEA_DIR}/experiments/preproc_finngen.py" \
        "${FINNGEN_GZ}" \
        "${CHAIN_FILE}" \
        "${BFILE}.bim" \
        "${LIFTOVER_BIN}" \
...
python3 "${LSEA_DIR}/experiments/compile_case_study.py" \
    --work_dir "${WORK_DIR}" \
    --phenotype "${PHENOTYPE}" \
...
log "  Summary: ${PHENOTYPE}_comparison_table.tsv"
